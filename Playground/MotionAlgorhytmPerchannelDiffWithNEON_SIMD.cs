...
namespace MMALSharp.Processors.Motion
{
    ...
    /// </summary>
    public class MotionAlgorhytmPerchannelDiffWithNEON_SIMD : MotionAlgorithmBase, IMotionAlgorithm
    {
        ...
        /// Constructor.
        /// </summary>
        /// <param name="perChannelTrheshold">The minimum RGB difference to indicate the pixel has changed. Maximum value is 765 (full white to full black).</param>
        ...
        public MotionAlgorhytmPerchannelDiffWithNEON_SIMD(int perChannelTrheshold = 200, int cellPixelPercentage = 50, int cellCountThreshold = 20)
        {
            // RGB 255 x 3 = 765
            if (perChannelTrheshold > 765)
            {
            ...
            _parameters.PerchannelThreshold = perChannelTrheshold;
        ...
        private void CheckDiff(long cellIndex, FrameDiffDriver driver, FrameAnalysisMetadata metadata, ThreadSafeParameters parameters)
        {
            ...
            for (var col = rect.X; col < x2; col++)
            {
                for (var row = rect.Y; row < y2; row++)
                {
                    ...
                    if (rgbDiff > parameters.PerchannelThreshold)
                    {
                        diff++;
                    }
        ...
        private struct ThreadSafeParameters
        {
            ...
            public int PerchannelThreshold;
